 ...
 * 
 */
public class EditarCarro extends com.example.aula04exemplo.provider.provider.api.copy.EditarCarro {
	
	private static final String TAG = "posgrad-fai";

	@Override
	protected Carro buscarCarro(long id) {
		// content://com.example.bancodados.provider.carro/carros/id
	...
	protected void salvarCarro(Carro carro) {
		Log.i(TAG, "Inserindo novo carro, nome: " + carro.nome);
		...
		if (id == 0) {
			...
			Log.i(TAG, "Novo carro salvo com sucesso, Uri: " + uri);
		} else {
			// Atualizar

			Uri uri = Carros.getUriId(id);
			Log.i(TAG, "Atualizando carro Uri: " + uri);

			int count = getContentResolver().update(uri, values, null, null);
			Log.i(TAG, "Carro atualizado com sucesso: " + count);
		}
	}
	...
	protected void excluirCarro(long id) {
		...
		Log.i(TAG, "Deletando carro Uri: " + uriCarro);

		int count = getContentResolver().delete(uriCarro, null, null);

		Log.i(TAG, "Carro excluido com sucesso: " + count);
	}
}
