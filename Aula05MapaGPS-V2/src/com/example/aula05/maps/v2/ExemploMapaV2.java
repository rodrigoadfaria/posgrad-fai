		...
		if(map == null) {
			...
			if(map != null) {
				...
				final CameraPosition position = new CameraPosition.Builder()
						...
						.tilt(90) 			// Ângulo (inclinação) que a câmera está 
...
//				adicionaPolyline(map, latLng, latLng2);
	...
	private void adicionarMarcador(GoogleMap map, LatLng latLng) {
		...
		markerOptions.position(latLng).title("Pós graduação FAI").snippet(
				"Desenvolvimento Ágil de Aplicativos para Web e Dispositivos Móveis");
		...
		map.setOnMarkerClickListener(new OnMarkerClickListener() {
			...
			public boolean onMarkerClick(Marker marker) {
				// Chamado quando o usuário clica no marcador
		...
		map.setOnInfoWindowClickListener(new OnInfoWindowClickListener() {
			...
			public void onInfoWindowClick(Marker marker) {
				// Chamado quando o usuário clica na janela do marcador
